...
const WorkBench: React.FC<Props> = ({ language, config, onUpdateConfig, onBack }) => {
  // 安全获取翻译
  const t = TRANSLATIONS[language] || TRANSLATIONS[Language.ZH_TW];
      ...
      canvasObjects.forEach(obj => {
        ...
        if (obj.isSelected && !isExporting) {
          ...
          corners.forEach((corner) => {
              ...
              ctx.lineWidth = lineWidth;
              ctx.beginPath();
              ctx.rect(corner.x - handleRad, corner.y - handleRad, handleRad * 2, handleRad * 2);
              ctx.fill();
              ctx.stroke();
          });
        }
  ...
  // 计算动态热区大小（保证小对象可操作）
  const getHitSize = (obj: CanvasObject) => {
      // 基础大小：手柄大小 * 热区倍数 / 视图缩放
      const baseHit = HANDLE_SIZE * HIT_SLOP / view.scale;
      // 限制大小：不能超过对象最小边长的 1/3 (这样中间留出 1/3 用于移动)
      const maxLimit = Math.min(obj.width, obj.height) / 3;
      return Math.min(baseHit, maxLimit);
  };
  ...
  const handlePointerDown = (e: React.MouseEvent | React.TouchEvent) => {
    ...
    if ('touches' in e && e.touches.length === 2) {
      ...
      setInteraction({
        ...
        startPoint: { x: 0, y: 0 }, 
        startObjectParams: { x: 0, y: 0, width: 0, height: 0, rotation: 0 }, 
        startPinchDist: getTouchDistance(t1, t2),
        startPinchCenter: getTouchCenter(t1, t2, rect),
        startView: { ...view }
      });
      return;
    }
    ...
    const clientPos = 'touches' in e ? { x: e.touches[0].clientX, y: e.touches[0].clientY } : { x: (e as React.MouseEvent).clientX, y: (e as React.MouseEvent).clientY };
    const mouseX = clientPos.x - rect.left;
    const mouseY = clientPos.y - rect.top;
    const { x: worldX, y: worldY } = screenToWorld(mouseX, mouseY);

    // 1. 优先检查【当前选中对象】的【手柄】
    // 只有手柄具有最高优先级，移动区域不在此列
    if (selectedObjectId) {
        const obj = canvasObjects.find(o => o.id === selectedObjectId);
        if (obj) {
            const { x: localX, y: localY } = worldToLocal(worldX, worldY, obj);
            const hitSize = getHitSize(obj); // 动态热区
            const rotateOffset = ROTATE_HANDLE_OFFSET / view.scale;
            
            // A. 旋转手柄 (圆形头部)
            if (Math.hypot(localX, localY - (-obj.height / 2 - rotateOffset)) <= Math.max(hitSize, 20/view.scale)) {
                setInteraction({
                    mode: 'rotate',
    ...
    // 2. 全图对象检测 (查找点击的最上层对象)
    // 倒序遍历，模拟 Z-index 从高到低
    let topHitObj: CanvasObject | null = null;
    ...
    if (topHitObj) {
        // 如果点击的不是当前选中的，切换选中
        if (topHitObj.id !== selectedObjectId) {
            setSelectedObjectId(topHitObj.id);
            setCanvasObjects(prev => prev.map(o => ({ ...o, isSelected: o.id === topHitObj!.id })));
        }
  ...
  const handlePointerUp = () => {
    if (selectedObjectId && interaction.mode !== 'none' && interaction.mode !== 'pan' && interaction.mode !== 'pinch') {
        const obj = canvasObjects.find(o => o.id === selectedObjectId);
  ...
  const updateCursor = (mouseX: number, mouseY: number) => {
    ...
    let hitAny = false;
    for (let i = canvasObjects.length - 1; i >= 0; i--) {
        if (isPointInObject(canvasObjects[i], worldX, worldY)) {
            hitAny = true;
            break;
        }
    }
    ...
    if (hitAny) {
        ...
        canvasRef.current.style.cursor = bgImage ? 'grab' : 'default';
    }
  };

  // ... 剩余代码保持不变，包括 handlePhotoUpload, handleWheel, handleDropTattoo, etc ...
  ...
  const handlePhotoUpload = async (file: File) => {
    ...
    try {
      ...
      reader.onload = (e) => {
        ...
        img.onerror = () => { setIsPhotoLoading(false); showToast('error', t.errorLoad || "Load Error"); };
        img.src = imgSrc;
      };
      reader.readAsDataURL(compressedFile);
    } catch (err) { setIsPhotoLoading(false); showToast('error', t.processImageFail || "Image processing failed"); }
  };
  ...
  const handleTattooClick = (tattoo: Tattoo) => {
    if (!bgImage || !canvasRef.current) { showToast('info', t.uploadTitle); return; }
  ...
  const handleDoubleClick = (e: React.MouseEvent) => {
    ...
    for (let i = canvasObjects.length - 1; i >= 0; i--) {
        if (isPointInObject(canvasObjects[i], worldX, worldY)) {
            clickedObjId = canvasObjects[i].id;
            break;
        }
    }
    if (clickedObjId) {
  ...
  const handleGenerate = async () => {
    ...
    } catch (error) { console.error("Fusion failed:", error); setIsProcessing(false); showToast('error', t.genFail); }
  };
  ...
  const handleTattooDragStart = (e: React.DragEvent, tattoo: Tattoo) => {
    ...
    const img = new Image();
    img.src = tattoo.imageBase64;
    img.onload = () => { previewImageRef.current = img; setDragPreviewTattoo(tattoo); };
  };
  ...
  const handleDragOver = (e: React.DragEvent) => {
    ...
    if (dragPreviewTattoo && canvasRef.current && bgImage) {
        ...
        const rect = canvasRef.current.getBoundingClientRect();
        setDragOverPos({ x: e.clientX - rect.left, y: e.clientY - rect.top });
    } else { e.dataTransfer.dropEffect = 'copy'; }
  };

  const handleDragLeave = (e: React.DragEvent) => { e.preventDefault(); setDragOverPos(null); };

  const handleDrop = (e: React.DragEvent) => {
    e.preventDefault();
    const tattooData = e.dataTransfer.getData('tattoo');
    if (tattooData) { handleDropTattoo(e, tattooData); return; }
  ...
  return (
    <div className={`flex h-screen w-full bg-background text-txt-primary font-sans overflow-hidden ${layoutClass}`}>
      ...
      {showGuide && (
        <div className="fixed inset-0 z-50 bg-black/60 flex items-center justify-center p-4 backdrop-blur-sm animate-fade-in">
            <div className="bg-surface rounded-2xl p-8 max-w-md w-full shadow-2xl relative">
                ...
                <button onClick={closeGuide} className="w-full bg-primary text-white py-3 rounded-xl font-semibold hover:bg-primary-hover transition-colors">{t.gotIt}</button>
            </div>
        </div>
      )}
      <input ref={fileInputRef} type="file" className="hidden" accept="image/png, image/jpeg, image/webp" onChange={(e) => e.target.files?.[0] && handlePhotoUpload(e.target.files[0])} />
      <div className="flex-1 flex flex-col relative min-h-0">
        <div className="h-14 bg-surface border-b border-border flex items-center justify-between px-4 shadow-sm z-10 shrink-0">
          <div className="flex items-center gap-2">
            ...
            {hasUserKey ? (
              <span className="text-xs bg-green-100 text-green-700 px-2 py-0.5 rounded ml-1 border border-green-200 font-medium whitespace-nowrap hidden sm:inline-block">Pro</span>
            ) : (
          ...
          <div className="flex items-center gap-2 md:gap-3">
             {bgImage && (
                 <>
                   <button onClick={handleUndo} disabled={historyIndex <= 0} className={`p-2 rounded-lg transition-colors ${historyIndex <= 0 ? 'text-txt-muted cursor-not-allowed' : 'text-txt-secondary hover:text-primary hover:bg-surface-highlight'}`} title={t.undo}>
                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 10h10a8 8 0 018 8v2M3 10l6 6m-6-6l6-6" /></svg>
                   </button>
                   <button onClick={handleRedo} disabled={historyIndex >= history.length - 1} className={`p-2 rounded-lg transition-colors ${historyIndex >= history.length - 1 ? 'text-txt-muted cursor-not-allowed' : 'text-txt-secondary hover:text-primary hover:bg-surface-highlight'}`} title={t.redo}>
                   ...
                   <button onClick={handleResetView} className="p-2 text-txt-secondary hover:text-primary hover:bg-surface-highlight rounded-lg transition-colors hidden sm:block" title={t.resetView}>
                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 8V4m0 0h4M4 4l5 5m11-1V4m0 0h-4m4 0l-5 5M4 16v4m0 0h4m-4 0l5-5m11 5l-5-5m5 5v-4m0 4h-4" /></svg>
                   </button>
                   <button onClick={handleDeleteOrReset} className={`p-2 rounded-lg transition-colors ${selectedObjectId ? 'text-red-500 bg-red-50 hover:bg-red-100 ring-1 ring-red-200' : 'text-txt-secondary hover:text-red-500 hover:bg-red-50'}`} title={selectedObjectId ? t.delete : t.reset}>
                     <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16" /></svg>
                   </button>
                   <button onClick={handleDownload} className="p-2 text-txt-secondary hover:text-primary hover:bg-surface-highlight rounded-lg transition-colors" title={t.download}>
             ...
             <button onClick={handleGenerate} disabled={!bgImage || canvasObjects.length === 0 || isProcessing} className={`px-4 py-2 rounded-lg text-white text-sm font-medium transition-all shadow-sm flex items-center gap-2 ${!bgImage || canvasObjects.length === 0 ? 'bg-divider cursor-not-allowed' : 'bg-primary hover:bg-primary-hover hover:shadow-md active:scale-95'}`}>
               {isProcessing ? (
                   <>
        ...
        <div ref={containerRef} className="flex-1 bg-background flex items-center justify-center p-4 relative overflow-hidden touch-none" onDragOver={handleDragOver} onDragLeave={handleDragLeave} onDrop={handleDrop} onWheel={handleWheel}>
          ...
          <canvas ref={canvasRef} id="photo-canvas" className="bg-white shadow-xl max-w-full max-h-full cursor-crosshair touch-none transition-shadow duration-300" onMouseDown={handlePointerDown} onMouseMove={handlePointerMove} onMouseUp={handlePointerUp} onMouseLeave={handlePointerUp} onTouchStart={handlePointerDown} onTouchMove={handlePointerMove} onTouchEnd={handlePointerUp} onDoubleClick={handleDoubleClick} />
          {!bgImage && !isPhotoLoading && (
            <div className="absolute inset-0 flex flex-col items-center justify-center cursor-pointer z-10 hover:bg-black/5 transition-colors group" onClick={triggerFileSelect}>
      ...
      <div className={`${galleryClass} z-20 bg-surface transition-all duration-300`}>
        <TattooGallery language={language} onDragStart={handleTattooDragStart} onTattooClick={handleTattooClick} isMobile={isMobile} />
      </div>
    </div>
  );
};
